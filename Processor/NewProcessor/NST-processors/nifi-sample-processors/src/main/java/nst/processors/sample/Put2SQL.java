...
public class Put2SQL extends AbstractSessionFactoryProcessor {
    ...
    private GroupingFunction groupFragmentedTransaction = (context, session, fc, conn, flowFiles, groups, sqlToEnclosure, result) -> {
        ...
        for (final FlowFile flowFile : flowFiles) {
            final String sql = context.getProperty(Put2SQL.SQL_STATEMENT).isSet()
                    ? context.getProperty(Put2SQL.SQL_STATEMENT).evaluateAttributeExpressions(flowFile).getValue()
            ...
            for (final FlowFile flowFile : enclosure.getFlowFiles()) {
                ...
                exceptionHandler.execute(fc, flowFile, input -> {
                    try (final PreparedStatement stmt = targetEnclosure.getNewStatement(conn, fc.obtainKeys)) {
                        ...
                        stmt.executeUpdate();
                        getLogger().info("Info After performing executeUpdate()");
                        getLogger().warn("Warning After performing executeUpdate()");
                        
                        getLogger().debug("Debug After performing executeUpdate()");
        ...
        for (final Map.Entry<String, String> entry : attributes.entrySet()) {
            ...
            if (matcher.matches()) {
                ...
                try {
                    ...
                    getLogger().info("Info In setParameter()");
                    getLogger().warn("Warning In setParameter()");                    
                    getLogger().debug("Debug In setParameter()");
